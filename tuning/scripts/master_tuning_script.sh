...
if [ ! -d "${WORKING_PATH}/library" ] || ${REDO}; then
  ...
  echo "make directory non-empty. Assuming ./provision_tuning.sh done previously"
...
if [ ! -d "${WORKING_PATH}/library" ] || ${REDO}; then
  ...
  echo "library directory non-empty. Assuming ./provision_verification.sh done previously"
  echo "Use --redo to force redoing previously done steps"
fi
...
if [ ! -d "${WORKING_PATH}/analysis" ] || ${REDO}; then
  ...
  echo "analysis directory non-empty. Assuming ./analyze_results.sh done previously"
  echo "Use --redo to force redoing previously done steps"
fi
